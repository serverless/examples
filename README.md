...
| [AWS MCP Server on Lambda (NodeJS)](https://github.com/serverless/examples/tree/v4/aws-node-mcp-server) <br/> Run an MCP server built with the official MCP TypeScript SDK on AWS Lambda, with streaming responses through API Gateway. | nodeJS |
| [AWS MCP Server on Lambda Function URL (NodeJS)](https://github.com/serverless/examples/tree/v4/aws-node-mcp-server-function-url) <br/> Run an MCP server built with the official MCP TypeScript SDK on a Lambda Function URL with response streaming - no API Gateway. | nodeJS |
