...
def create_user():
    ...
    if status in ('DAILY-SMS', 'STOP-SERVICES'):
        #dynamodb_client.put_item(TableName=USERS_TABLE, Item={'phone_number': {'S': phone_number}, 'status': {'S': status}})
        response = dynamodb_client.update_item(TableName=USERS_TABLE,
        ...
        # dynamodb_client.update_item(TableName=USERS_TABLE, 
        #                             Key={
